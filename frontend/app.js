  ...
  try {
    ...
    document.getElementById('rail-metrics').innerHTML = `
      <div class="metric-card primary">
        ...
        <span class="metric-value">$${Number(dashboard.total_sent_usd || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
      </div>
      <div class="metric-card">
        <span class="metric-label">Transactions</span>
        <span class="metric-value">${dashboard.total_transactions || 0}</span>
      </div>
      <div class="metric-card warn">
        <span class="metric-label">Pending</span>
        <span class="metric-value">${dashboard.pending_count || 0}</span>
      </div>
      <div class="metric-card" style="border-left:3px solid var(--danger)">
        <span class="metric-label">Failed / Returned</span>
        <span class="metric-value">${dashboard.failed_count || 0}</span>
      </div>
      <div class="metric-card">
        <span class="metric-label">Total Fees</span>
        <span class="metric-value">$${Number(dashboard.total_fees_usd || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
      </div>
      <div class="metric-card">
        <span class="metric-label">Webhook Events</span>
        <span class="metric-value">${dashboard.webhook_events || 0}</span>
      </div>`;

    // Limits
    document.getElementById('rail-limits').innerHTML = `
      <div class="panel" style="flex:1">
        <h3>ACH Daily Limit</h3>
        <div style="display:flex;justify-content:space-between;margin-top:8px">
          <span>Used: <strong>$${Number(limits.ach.used_usd).toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong></span>
          <span>Remaining: <strong>$${Number(limits.ach.remaining_usd).toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong></span>
          <span>Limit: $${Number(limits.ach.daily_limit_usd).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
        </div>
        <div style="background:var(--bg-hover);border-radius:4px;height:8px;margin-top:8px;overflow:hidden">
          <div style="background:var(--accent);height:100%;width:${Math.min(100, (parseFloat(limits.ach.used_usd) / parseFloat(limits.ach.daily_limit_usd)) * 100)}%;border-radius:4px"></div>
        </div>
      </div>
      <div class="panel" style="flex:1">
        <h3>Wire Daily Limit</h3>
        <div style="display:flex;justify-content:space-between;margin-top:8px">
          <span>Used: <strong>$${Number(limits.wire.used_usd).toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong></span>
          <span>Remaining: <strong>$${Number(limits.wire.remaining_usd).toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong></span>
          <span>Limit: $${Number(limits.wire.daily_limit_usd).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
        </div>
        <div style="background:var(--bg-hover);border-radius:4px;height:8px;margin-top:8px;overflow:hidden">
          <div style="background:var(--accent);height:100%;width:${Math.min(100, (parseFloat(limits.wire.used_usd) / parseFloat(limits.wire.daily_limit_usd)) * 100)}%;border-radius:4px"></div>
        </div>
      </div>`;

    loadRailTransactions();
  } catch (err) {
    document.getElementById('rail-metrics').innerHTML = `<p style="padding:20px;color:var(--danger)">Error: ${err.message}</p>`;
  }
}

async function loadRailTransactions() {
  try {
    ...
    html += '</tbody></table>';
    document.getElementById('rail-transactions-table').innerHTML = html;
  } catch (err) {
    document.getElementById('rail-transactions-table').innerHTML = `<p style="padding:20px;color:var(--danger)">Error: ${err.message}</p>`;
  }
}

function railTxActions(tx) {
  let btns = '';
  if (tx.status === 'pending_approval') {
    btns += `<button class="btn btn-sm btn-success" onclick="approveRailTx(${tx.id})">Approve</button> `;
  }
...
async function approveRailTx(id) {
  try {
    const data = await api(`/payment-rails/transactions/${id}/approve`, { method: 'POST', body: JSON.stringify({ approved_by: 'dashboard_user' }) });
    showToast(data.message || 'Transaction approved');
    loadRailTransactions();
  } catch (err) { showToast(err.message, 'error'); }
}

async function cancelRailTx(id) {
  if (!confirm('Cancel this payment rail transaction?')) return;
  try {
    await api(`/payment-rails/transactions/${id}/cancel`, { method: 'POST', body: JSON.stringify({ reason: 'Cancelled by user' }) });
...
async function showSendPaymentRail() {
  ...
  const accountsData = await api('/accounts');
  const active = (accountsData.accounts || []).filter(a => a.status === 'active');
  document.getElementById('rail-from-account').innerHTML =
    active.map(a => `<option value="${a.id}">${a.account_number} — ${a.account_name} (${formatUSD(a.balance_cents)})</option>`).join('');

  // Populate contacts
  try {
    const contactsData = await api('/crm/contacts?status=active');
    const contacts = (contactsData.contacts || []).filter(c => c.status === 'active');
    document.getElementById('rail-contact').innerHTML =
      '<option value="">Enter recipient details manually...</option>' +
      contacts.map(c => `<option value="${c.id}">${c.display_name || (c.first_name + ' ' + c.last_name)} (${c.contact_type})</option>`).join('');
  } catch (_) {}

  toggleRailFields();
  document.getElementById('send-rail-modal').classList.remove('hidden');
}

async function loadRailContactInfo() {
  const contactId = document.getElementById('rail-contact').value;
  const manualFields = document.getElementById('rail-manual-recipient');
  if (!contactId) {
    manualFields.classList.remove('hidden');
    return;
  }
  // If contact selected, hide manual fields and auto-populate from CRM
  try {
    const data = await api(`/crm/contacts/${contactId}`);
    const contact = data.contact;
    const pms = data.payment_methods || [];
    const defaultPm = pms.find(p => p.is_default) || pms[0];
    if (defaultPm) {
      document.querySelector('#send-rail-form [name="recipient_name"]').value = contact.display_name || '';
      document.querySelector('#send-rail-form [name="recipient_bank"]').value = defaultPm.bank_name || '';
      document.querySelector('#send-rail-form [name="recipient_routing"]').value = defaultPm.routing_number || '';
      document.querySelector('#send-rail-form [name="recipient_account"]').value = defaultPm.account_number || '';
      manualFields.classList.remove('hidden');
    } else {
      manualFields.classList.remove('hidden');
    }
...
async function sendRailPayment(e) {
  e.preventDefault();
  const form = new FormData(e.target);
  const body = {
    ...
    amount: parseFloat(form.get('amount')),
    from_account_id: parseInt(form.get('from_account_id')),
    contact_id: form.get('contact_id') ? parseInt(form.get('contact_id')) : null,
    recipient_name: form.get('recipient_name') || null,
    recipient_bank: form.get('recipient_bank') || null,
    recipient_routing: form.get('recipient_routing') || null,
    recipient_account: form.get('recipient_account') || null,
    description: form.get('description') || null,
    memo: form.get('memo') || null,
    sec_code: form.get('sec_code') || null,
    settlement_schedule: form.get('settlement_schedule') || null,
  };

  // Parse check mailing address
  if (body.rail === 'check') {
    try {
      body.mailing_address = JSON.parse(form.get('mailing_address_json') || '{}');
    } catch (_) {
      showToast('Invalid mailing address JSON', 'error');
      return;
    }
  }

  try {
    const data = await api('/payment-rails/send', { method: 'POST', body: JSON.stringify(body) });
    showToast(data.message);
    hideModal('send-rail-modal');
    e.target.reset();
    loadPaymentRails();
  } catch (err) { showToast(err.message, 'error'); }
}

async function showRailConfig() {
  try {
    const data = await api('/payment-rails/config');
    ...
    html += '</div>';
    html += '<button class="btn btn-primary" onclick="saveRailConfigs()" style="margin-top:12px">Save All</button>';
    document.getElementById('rail-config-form').innerHTML = html;
    document.getElementById('rail-config-modal').classList.remove('hidden');
  } catch (err) { showToast(err.message, 'error'); }
}

async function saveRailConfigs() {
  const inputs = document.querySelectorAll('#rail-config-form input[data-config-key]');
  let saved = 0;
  for (const input of inputs) {
    if (input.readOnly) continue;
    try {
      await api('/payment-rails/config', {
        method: 'PUT',
        body: JSON.stringify({ key: input.dataset.configKey, value: input.value }),
      });
      saved++;
    } catch (_) {}
  }
  showToast(`${saved} config(s) saved`);
  hideModal('rail-config-modal');
  loadPaymentRails();
}
