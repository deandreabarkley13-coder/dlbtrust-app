...
## Trustee portal wire approvals (`/dapp/trust-dashboard.html`, maker/checker dual control)

Use this for anything involving `wire_transfers`, `status = 'pending_approval'`, micro-deposit test wires,
or the trustee dashboard's **Wire Approvals** tab.

### Local server for trustee-portal testing

```bash
setsid nohup env \
  DATABASE_URL=postgres://dlbtrust:dlbtrust@localhost:5432/dlbtrust \
  JWT_SECRET=dev-jwt-secret-wire-test \
  ADMIN_SECRET_TOKEN=dlb-admin-2026-trust \
  PAYMENT_DATA_ENCRYPTION_KEY=$(openssl rand -hex 32) \
  DAPP_OTP_ALWAYS_SHOW_CODE=true PORT=3002 \
  node server/server-3002.js > /tmp/server-3002-wire.log 2>&1 < /dev/null & disown
```

- The trustee/beneficiary portal login is at `/trust-portal/index.html` (and the left card of `/dapp`);
  the trustee dashboard itself is served at `/dashboard`.
- Login is email + one-time PIN. The PIN is not shown in the UI; read it from the DB right before entering it:
  `PGPASSWORD=dlbtrust psql -U dlbtrust -h localhost -d dlbtrust -t -c "select otp_code from dapp_users where email='<email>';"`
  (Click **Send PIN** first — it rotates `otp_code`.)
- Portal JWT is stored in `localStorage` under `dlb-beneficiary-token` (not `dlb-admin-token`, which is the
  operator dashboard's admin token). Grab it from the console when you need to replay an authenticated API call
  with the exact seat the browser is using.
- Default seats (`server/integrations/dapp/trustees.js`): maker `malissa1130@gmail.com`,
  checker `deandreabarkley13@gmail.com` (overridable via `TRUST_MAKER_EMAIL` / `TRUST_CHECKER_EMAIL`).
  The JWT carries `role` / `roles` values `trustee_maker` / `trustee_checker`, which is what
  `server/routes/wire.js` maps to a maker/checker seat.
- If `wire_transfers` / `dapp_users` do not exist yet, just start the server once — startup creates them.

### Seeding pending wires

```bash
curl -s -X POST -H 'x-admin-token: dlb-admin-2026-trust' -H 'Content-Type: application/json' \
  -d '{"amount":0.25,"beneficiaryName":"DeAndrea Barkley","accountNumber":"692101092959","routingNumber":"121145307","bankName":"Lili Bank","description":"Micro-deposit test wire A"}' \
  http://localhost:3002/api/wire/initiate
```

- Wires created with the admin token get `initiated_by = 'admin'` and land in `pending_approval`.
- To exercise the "own wire" case, insert/initiate a wire with `initiated_by` set to the checker's email.

### Dual-control rules worth asserting

- Checker JWT can approve an admin-initiated wire: status becomes `approved` and
  `wire_transfers.approved_by` is the **checker's email** (not `admin`/`checker`).
- Maker JWT: `POST /api/wire/:id/approve` and `/reject` → HTTP **403**
  `Only the checker trustee can approve/reject a wire`; the maker can still *read* `/api/wire/pending-approvals` (200).
- Beneficiary-only JWT → HTTP **401** `Authentication required. Use x-admin-token...`.
- `x-admin-token` approval of an `initiated_by='admin'` wire → HTTP **400**
  `Maker cannot approve their own wire (dual-control violation)`. This is why the trustee-session path exists;
  don't mistake it for a regression.

### UI gotcha

- Disabled Approve/Reject buttons in the Wire Approvals tab keep their normal full-color styling
  (the `disabled` attribute is set but there is no greyed-out CSS), so a screenshot alone looks "enabled".
  Confirm the disabled state via the DOM (`disabled="true"`) and by clicking to show it is a no-op.
- The Settlement Gateway tab may show `duplicate key value violates unique constraint "pg_class_relname_nsp_index"`
  in its summary card on a fresh local DB (concurrent `CREATE TABLE IF NOT EXISTS` race at startup); this is
  unrelated to wire approvals and can be ignored/retried after a reload.
