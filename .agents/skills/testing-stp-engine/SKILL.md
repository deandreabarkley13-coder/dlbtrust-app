...
- **Fixed (PR #178)**: UI now accesses `st = d.stats || d` and reads `st.clearing`, `st.pending_volume`, etc.
- If entries exist (submit a deposit first via Test 3), "Clearing" should show a non-zero count and "Pending Volume" should show the dollar amount
- **Pass**: Stats reflect actual data (e.g. Clearing=1, Pending Volume=$5.00 when 1 entry exists)
- **Fail**: All stats show 0/$0.00 despite entries existing in the STP table below
