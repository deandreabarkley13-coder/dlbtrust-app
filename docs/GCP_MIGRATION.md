...
- **Payment Gateway OS** — wired; `shadow` on the deployed project
  (`PAYMENT_GATEWAY_LIVE=false`). Go-live steps for trust distributions /
  disbursements: (1) bring the Payment Processor OS to `live` first (steps
  above) — the gateway dispatches through it; (2) seed
  `PAYMENT_GATEWAY_WEBHOOK_SECRET` (share it with the processor's callback
  configuration) and keep `PAYMENT_DATA_ENCRYPTION_KEY` /
  `STRIPE_PAYMENTS_SECRET_KEY` in `secret_names`; (3) set
  `runtime_environment.PAYMENT_GATEWAY_LIVE="true"` leaving
  `PAYMENT_GATEWAY_REQUIRE_APPROVAL_REF`, `PAYMENT_GATEWAY_REQUIRE_SCREENING_REF`
  and `PAYMENT_GATEWAY_REQUIRE_DISTRIBUTION_REQUEST` at `"true"` (optionally
  cap `PAYMENT_GATEWAY_MAX_DISBURSEMENT_CENTS`) and `terraform apply`;
  (4) confirm `GET /api/os/readiness/payment-gateway` reports `mode: live`
  with an empty `blockers` list and `GET /api/os/readiness` returns 200.
  Operating sequence per payout: beneficiary/trustee raises a distribution
  request (`/api/dapp/distributions`), both trustees approve it, the maker
  `tokenize`s the beneficiary's payout method once and `submit`s an intent
