 ...
 * external account whose last4 (and, when known, routing) match the
 * registered Lili account (LILI_DD_*); no other bank account is ever paid.
 * Fails closed on test-mode keys, payouts disabled, no matching external
 * account, or insufficient available balance.
 */
...
class LiliStripePayoutOriginator {
  static getConfig() {
    ...
    return {
      ...
      allowTestMode: String(env.LILI_STRIPE_PAYOUT_ALLOW_TEST || 'false').toLowerCase() === 'true',
      _key: key,
    };
  }
  ...
  /** The Stripe external bank account that is the registered Lili account. */
  static _matchExternalAccount(externalAccounts, dest, cfg) {
    const list = Array.isArray(externalAccounts) ? externalAccounts.filter((a) => a.object === 'bank_account') : [];
    const wantLast4 = last4(dest._account);
    const matches = list.filter((a) => a.last4 === wantLast4 && (!cfg.externalAccountId || a.id === cfg.externalAccountId));
    ...
    if (!stripe) issues.push('stripe npm package not installed');
      ...
      try {
        ...
        const [acct, bal] = await Promise.all([
          client.accounts.retrieve({ expand: ['external_accounts'] }),
        ...
        const ea = dest && dest.configured
          ? this._matchExternalAccount(acct.external_accounts && acct.external_accounts.data, dest, cfg)
          : null;
        ...
        if (bal.livemode !== true && !cfg.allowTestMode) issues.push('Stripe balance is test-mode');
  ...
  static async send({ amount, reference, description } = {}) {
    ...
    return {
      ...
      expectedArrival: payout.arrival_date ? new Date(payout.arrival_date * 1000).toISOString().slice(0, 10) : null,
    };
  }
}
