 ...
 * via the self-hosted Moov Paygate OS engine rail.
 *
 * Examples:
 *   node server/scripts/sendPtcMoovPaygatePayment.js --amount 5000 --sourceAccountId 4000 --routing 121145307 --account 692101092959 --name "DB NET MGMT" --bankName "Lili Bank"
 *
 * The script sources funds from the interest-income GL (4000) into the PTC bank
 * ledger, then originates a Moov Paygate transfer to credit the payee account.
 */
...
async function main() {
  ...
  if (!Number.isFinite(amount) || amount <= 0) {
    throw new Error('Usage: --amount <dollars> [--sourceAccountId 4000] [--fromAccountId <id>] [--routing 121145307] [--account 692101092959] [--name "DB NET MGMT"] [--bankName "Lili Bank"]');
  }
  ...
  const routing = args.routing || process.env.MOOV_PAYGATE_DEST_ROUTING || '121145307';
  const account = args.account || process.env.MOOV_PAYGATE_DEST_ACCOUNT || '692101092959';
  const name = args.name || process.env.MOOV_PAYGATE_DEST_NAME || 'DB NET MGMT';
  const bankName = args.bankName || process.env.MOOV_PAYGATE_DEST_BANK_NAME || 'Lili Bank';
  ...
  const payload = {
    ...
    description: args.description || `Moov Paygate payment from ${sourceAccountId} to ${name}`,
    initiatedBy: args.initiatedBy || process.env.PTC_BANK_INITIATED_BY || 'ptc-moov-script',
    autoSend: true,
    senderRouting,
    senderAccount,
    sameDay: args.sameDay === 'true',
    triggerCutoff: args.triggerCutoff !== 'false',
    metadata: {
      senderRouting,
      senderAccount,
      senderName: args.senderName || process.env.PTC_BANK_NAME || process.env.MOOV_PAYGATE_ORIGINATOR_NAME || 'DLB Trust PTC Bank',
      sameDay: args.sameDay === 'true',
      triggerCutoff: args.triggerCutoff !== 'false',
  ...
  console.log('Originating PTC Moov Paygate payment with payload:');
  ...
  if (!result.success || !result.result?.sent) {
    console.warn('Moov Paygate payment did not complete; see result above.');
    process.exitCode = 1;
  }
}
