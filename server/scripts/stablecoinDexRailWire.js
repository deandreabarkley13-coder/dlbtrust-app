 ...
 *
 * Fail-closed: --create-pool and --fund refuse to run unless both readiness
 * checks are ready (live mode, not shadow). Secret values are never printed.
 */
...
function parseArgs(argv) {
  const out = { readiness: false, createPool: false, fund: false, target: 'USDC', seedDlbusd: null, seedUsdc: null, amount: null, sourceType: null, sourceAccount: null };
...
async function run(argv, { Dex = StablecoinDexEngine, Deposit = TreasuryDepositEngine, log = console.log, error = console.error } = {}) {
  ...
  if (!args.createPool && !args.fund) {
    if (!args.readiness) log('\nno --create-pool/--fund given; nothing moved.');
